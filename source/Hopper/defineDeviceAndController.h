...
#include <Simulation/Model/Actuator.h>
...
namespace OpenSim
{
    ...
    //------------------------------------------------------------------------------
    // Device is a type of ModelComponent that contains all the parts comprising
    // the assistive device model (a PathActuator plus bodies and joints for
    // attaching the actuator to the hopper or testbed). Devices are built by
    // buildDevice() (see buildDeviceModel.cpp). [Step 2, Task A]
    //------------------------------------------------------------------------------
    ...
    public:
        ...
        OpenSim_DECLARE_OUTPUT(
            length, double, getLength, SimTK::Stage::Position);

        // The lengthening speed of the device.
        OpenSim_DECLARE_OUTPUT(
            speed, double, getSpeed, SimTK::Stage::Velocity);

        // The force transmitted by the device.
        OpenSim_DECLARE_OUTPUT(
            tension, double, getTension, SimTK::Stage::Acceleration);

        /* // The height of the model to which the device is attached. */
        OpenSim_DECLARE_OUTPUT(
            height, double, getHeight, SimTK::Stage::Position);

        // The center of mass height of the model to which the device is
        // attached.
        OpenSim_DECLARE_OUTPUT(
            com_height, double, getCenterOfMassHeight, SimTK::Stage::Position);

        // Member functions that access quantities in which we are interested.
        // These methods are used by the outputs declared above.
        ...
        double getTension(const SimTK::State& s) const
        {
            if (s.getSystemStage() == SimTK::Stage::Velocity) {
                std::cout << "Warning: Requesting tension during velocity stage" << std::endl;
                return 0.;
            }
            return getComponent<PathActuator>("cableAtoB").getOutputValue<double>(s, "tension");
        }
        double getHeight(const SimTK::State& s) const
        {
            const std::string hopperHeightCoord = "jointset/slider";
            return getModel().getComponent<SliderJoint>(hopperHeightCoord)
                .getCoordinate(SliderJoint::Coord::TranslationX).getValue(s);
        }
    ...
    protected:
        ...
        {
            // TODO this breaks the Stage constraint.
            /* const auto& actuator = getComponent<PathActuator>("cableAtoB"); */
            /* double level = */
            /*     fmin(1., getTension(s) / actuator.get_optimal_force()); */
            /* actuator.getGeometryPath().setColor( */
            /*     s, SimTK::Vec3(0.1, level, 0.1)); */
        }
    ...
    // Controller is intended to simulate a proportional myoelectric controller
    // [1], and can control any ScalarActuator (set using the "actuator"
    // socket). [1]
    // https://en.wikipedia.org/wiki/Proportional_myoelectric_control [Step 2,
    // Task B]
    //------------------------------------------------------------------------------
    ...
    public:
        ...
        OpenSim_DECLARE_SOCKET(
                actuator, // = name...
                ScalarActuator,
                "Socket for connecting scalar actuator");

        // Input the activation signal 'a' to which the controller's output
        // should be proportional.
        OpenSim_DECLARE_INPUT(
            activation, double, SimTK::Stage::Model,
            "The signal to which the controller's output is proportional");

        // Output the control signal generated by the controller.
        OpenSim_DECLARE_OUTPUT(
            myo_control, double, computeControl, SimTK::Stage::Time);
        ...
        double computeControl(const SimTK::State& s) const
        {
            ...
            return get_gain() * (0.5 - activation);
        }
