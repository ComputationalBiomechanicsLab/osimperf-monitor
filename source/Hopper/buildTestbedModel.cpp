...
namespace OpenSim
{
    ...
    //------------------------------------------------------------------------------
    // Build a testbed for testing the device before attaching it to the hopper.
    // We will attach one end of the device to ground ("/testbed/ground") and
    // the other end to a sprung load ("/testbed/load").
    //------------------------------------------------------------------------------
    Model buildTestbed(bool showVisualizer)
    {
        ...
        // Attach the load to ground with a FreeJoint and set the location of
        // the load to (1,0,0).
        auto gndToLoad = new FreeJoint("gndToLoad", testbed.getGround(), *load);
        gndToLoad->updCoordinate(FreeJoint::Coord::TranslationX)
            .setDefaultValue(1.0);
        ...
        auto spring = new PointToPointSpring(
            testbed.getGround(),
            Vec3(0),        // frame G and location in G of point 1
